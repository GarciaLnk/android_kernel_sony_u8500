 ...
 * ABBamp
 * 
 * Audio control module for ST-Ericsson AB8500 codec
 * 
...
/* 
 * Read bits from ab8500 codec registers
 * 
 * @reg:	codec register
...
static int abbamp_read(unsigned int reg, unsigned int shift, unsigned int width)
{
	...
	ret = snd_soc_read(ab8500_codec, reg);
	shift_mov = shift + 1;
 ...
 * abbamp_write
 * 
 * Writes masked bits to ab8500 code registers 
 * 
 * @reg:	codec register
 * @shift:	offset of bit(s)
 * @width:	width of bit(s)
...
static int abbamp_write(unsigned int reg, unsigned int shift, unsigned int width, unsigned int value)
{
	...
	obj = snd_soc_read(ab8500_codec, reg);
	...
	ret = snd_soc_write(ab8500_codec, reg, obj);

	return ret;
}

/* Codec ID */
static char *ab8500_codec_id;
...
static ssize_t abb_codec_vertag_show(struct kobject *kobj, 
		struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "Codec: %s\nABBamp: %s\ncocafe\n", ab8500_codec_id, ABBAMP_VERSION_TAG);
}
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	old = snd_soc_read(ab8500_codec, reg);

	snd_soc_write(ab8500_codec, reg, value);

	new = snd_soc_read(ab8500_codec, reg);
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "off", 3)) {
		...
		if (hs_ponup) {
			/* 0100 0100 */
			snd_soc_write(ab8500_codec, REG_ANAGAIN3, 0x44);
		}
	...
	if (!strncmp(&buf[0], "left=", 5)) {
		old = snd_soc_read(ab8500_codec, REG_ANAGAIN3);
		...
		new = snd_soc_read(ab8500_codec, REG_ANAGAIN3);
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "off", 3)) {
		...
		if (hs_ponup) {
			/* 0000 1000 */
			snd_soc_write(ab8500_codec, REG_HSLEARDIGGAIN, 0x08);
		}
	...
	if (!strncmp(&buf[0], "gain=", 5)) {
		old = snd_soc_read(ab8500_codec, REG_HSLEARDIGGAIN);
		ret = sscanf(&buf[5], "%d", &val);
		...
		new = snd_soc_read(ab8500_codec, REG_HSLEARDIGGAIN);
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "off", 3)) {
		...
		if (hs_ponup) {
			/* 0000 1000 */
			snd_soc_write(ab8500_codec, REG_HSRDIGGAIN, 0x08);
		}
	...
	if (!strncmp(&buf[0], "gain=", 5)) {
		old = snd_soc_read(ab8500_codec, REG_HSRDIGGAIN);
		ret = sscanf(&buf[5], "%d", &val);
		...
		new = snd_soc_read(ab8500_codec, REG_HSRDIGGAIN);
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(&buf[0], "mode=", 5)) {
		old = snd_soc_read(ab8500_codec, REG_ANACONF1);
		ret = sscanf(&buf[5], "%d", &val);
		...
		new = snd_soc_read(ab8500_codec, REG_ANACONF1);
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "off", 3)) {
		...
		if (ad2_ponup) {
			/* 0001 1111 */
			snd_soc_write(ab8500_codec, REG_ADDIGGAIN2, 0x1f);
		}
	...
	if (!strncmp(&buf[0], "gain=", 5)) {
		old = snd_soc_read(ab8500_codec, REG_ADDIGGAIN2);
		ret = sscanf(&buf[5], "%d", &val);
		...
		new = snd_soc_read(ab8500_codec, REG_ADDIGGAIN2);
...
static ssize_t abb_codec_chargepump_show(struct kobject *kobj, 
		struct kobj_attribute *attr, char *buf)
{
	int bit = (snd_soc_read(ab8500_codec, REG_SIGENVCONF)) & BIT(REG_SIGENVCONF_CPLVEN);
		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (sysfs_streq(buf, "0")) {
		regval = snd_soc_read(ab8500_codec, REG_SIGENVCONF);
		regval = regval & 0x1f;	
		snd_soc_write(ab8500_codec, REG_SIGENVCONF, regval);
